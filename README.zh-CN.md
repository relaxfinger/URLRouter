...
UniversalLink：校验并拆分 URL
              │
              ▼
RouteModule：Feature 自己的 URL 语法和 View 工厂
              │
              ▼
presentation query：描述展示方式
              │
              ▼
AppRouter：保存导航状态
              │
...
在模块化 App 中，每个 Feature 通过 `RouteModule` 自己维护 URL 语法和目标 View。URL 自身携带展示协议，`URLRouter` 负责校验、解析和主线程导航。
...
### 第一步：在 Feature Package 内注册路由

Feature Package 自己处理路径匹配和目标 View，主 App 不需要认识 `/articles/*`：
...
### 第二步：在 App Shell 一次性组装模块

App 只负责链接 Feature Package 并把模块交给注册表，不包含 URL 路径或展示方式映射：
...
仅负责跳转的 Feature View 只需要 `SwiftUI`，通过 `openURL` 发送完整协议 URL：
...
Swift Package 必须先被 App 链接，运行时不能自动发现未链接模块；因此新增 Package 时仍要添加依赖/注册模块，但不再需要修改中心化的 URL → 页面 `switch`。

## 旧版强类型路由接入

`UniversalLinkRoute`、`RoutePresentation` 和 `universalLinkRouting` 仍保留，适合小型 App 或迁移期；对于可独立演进的 Package，请使用上方模块注册表方案。
