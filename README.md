...
## Route contracts and plugin workflow

URLRouter maintains one App-root route contract, not one contract per Feature.
It scans both Feature Packages and App-owned Swift source, then produces two
...
an App-owned route, all four presentation styles, cross-package navigation, a
cache-first policy lifecycle, telemetry, and coordinated concurrent routes.
